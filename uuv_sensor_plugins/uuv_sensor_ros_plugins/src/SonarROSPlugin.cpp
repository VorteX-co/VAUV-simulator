...
/////////////////////////////////////////////////
void SonarROSPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  GetSDFParam<std::string>(_sdf, "beam_link_name", beamLinkName, "");
  GZ_ASSERT(!beamLinkName.empty(), "Beam  link name empty");
  this->beamsLinkNames.push_back(beamLinkName);

  // Load the beam output topic names
  std::string beamTopic;
  GetSDFParam<std::string>(_sdf, "beam_topic", beamTopic, "");
  GZ_ASSERT(!beamTopic.empty(), "Beam  topic name empty");
  this->beamTopics.push_back(beamTopic);


  // Create beam subscribers
  this->beamSub.reset(new message_filters::Subscriber<sensor_msgs::msg::Range>(
    myRosNode, this->beamTopic));

    this->dvlBeamMsg = uuv_sensor_ros_plugins_msgs::msg::DVLBeam();
  ...
  this->syncBeamMessages->registerCallback(
    std::bind(&SonarROSPlugin::OnBeamCallback, this, _1));
